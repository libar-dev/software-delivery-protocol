    ...
    if (
      ...
      !findings.some(
        (finding) =>
          ...
          finding.file?.endsWith(".feature") === true,
      )
    ) {
      writeStderr(
        output,
        `note: no *.sdp.ts, *.sdp.md, or *.feature spec files found under ${resolvedRoot} — the authored model is empty. Is this the right extraction root?\n`,
      );
    }
