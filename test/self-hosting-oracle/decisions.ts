...
export const decisionsSpecs = [
  ...
  },
  {
    ...
    sections: {
      intent: {
        outcome:
          "Rule that architecturally significant patterns and their relationships are authored on the existing Spec primitive, relation vocabulary, and structural anchors, so the graph answers architecture questions without a second architecture vocabulary.",
      },
      decision: {
        context:
        ...
        consequences: [
          ...
          "Two documented grain limits stand as named limits, not holes: pattern membership is Spec-grain (one `codeAnchor` carries one `satisfies`), and negative constraints are claimable but not enforced until the deferred architecture-enforcement validator family lands.",
