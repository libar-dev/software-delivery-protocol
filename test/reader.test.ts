...
describe("the reader — the thin typed loader behind the agent surface", () => {
  ...
  describe("honesty over foreign producers", () => {
    ...
    it("never decodes an anchored verifies edge from a non-Anchor source as the enabling test binding", () => {
      ...
      // off-contract (`spec:extraction.delivery-facts`: an anchored verifies edge resolves from an
      // Anchor node only).
    ...
    it("never decodes an off-contract verifies edge from an anchor-bound example as enabled", () => {
      ...
      // parent — off-contract (`spec:extraction.delivery-facts`: an example's verifies edge confers
      // the binding only as
