...
describe("the self-hosting corpus", () => {
  ...
  it("holds the frozen corpus totals", () => {
    ...
    expect(result.counts).toEqual({ specs: 103, packs: 1, anchors: 75 });
    expect(expectedSpecs).toHaveLength(103);
    expect(expectedPackMembers).toHaveLength(103);
    expect(expectedAnchors).toHaveLength(75);
    expect(result.graph.nodes).toHaveLength(179);
    expect(result.graph.edges).toHaveLength(351);
  });
