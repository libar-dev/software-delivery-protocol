...
describe("the self-hosting corpus", () => {
  ...
  it("holds the frozen corpus totals", () => {
    ...
    expect(result.counts).toEqual({ specs: 159, packs: 1, anchors: 157 });
    expect(expectedSpecs).toHaveLength(159);
    expect(expectedPackMembers).toHaveLength(159);
    expect(expectedAnchors).toHaveLength(157);
    expect(result.graph.nodes).toHaveLength(317);
    expect(result.graph.edges).toHaveLength(666);
  });
  ...
  it("holds the frozen stated-readiness distribution", () => {
    ...
    ).toEqual({ defined: 9, idea: 6, ready: 143, scoped: 1 });
  });
