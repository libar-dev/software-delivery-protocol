...
describe("the self-hosting corpus", () => {
  ...
  it("holds the frozen corpus totals", () => {
    ...
    expect(result.counts).toEqual({ specs: 149, packs: 1, anchors: 138 });
    expect(expectedSpecs).toHaveLength(149);
    expect(expectedPackMembers).toHaveLength(149);
    expect(expectedAnchors).toHaveLength(138);
    expect(result.graph.nodes).toHaveLength(288);
    expect(result.graph.edges).toHaveLength(545);
  });
  ...
  it("holds the frozen stated-readiness distribution", () => {
    ...
    ).toEqual({ defined: 14, idea: 3, ready: 131, scoped: 1 });
  });
