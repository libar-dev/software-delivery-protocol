...
describe("the self-hosting corpus", () => {
  ...
  it("holds the frozen corpus totals", () => {
    ...
    expect(result.counts).toEqual({ specs: 108, packs: 1, anchors: 80 });
    expect(expectedSpecs).toHaveLength(108);
    expect(expectedPackMembers).toHaveLength(108);
    expect(expectedAnchors).toHaveLength(80);
    expect(result.graph.nodes).toHaveLength(189);
    expect(result.graph.edges).toHaveLength(371);
  });
