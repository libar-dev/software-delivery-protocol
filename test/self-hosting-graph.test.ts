...
describe("the self-hosting corpus", () => {
  ...
  it("holds the frozen corpus totals", () => {
    ...
    expect(result.counts).toEqual({ specs: 160, packs: 1, anchors: 157 });
    expect(expectedSpecs).toHaveLength(160);
    expect(expectedPackMembers).toHaveLength(160);
    expect(expectedAnchors).toHaveLength(157);
    expect(result.graph.nodes).toHaveLength(318);
    expect(result.graph.edges).toHaveLength(672);
  });
  ...
  it("holds the frozen stated-readiness distribution", () => {
    ...
    ).toEqual({ defined: 9, idea: 6, ready: 144, scoped: 1 });
  });
