...
describe("the self-hosting corpus", () => {
  ...
  it("holds the frozen corpus totals", () => {
    ...
    expect(result.counts).toEqual({ specs: 93, packs: 1, anchors: 70 });
    expect(expectedSpecs).toHaveLength(93);
    expect(expectedPackMembers).toHaveLength(93);
    expect(expectedAnchors).toHaveLength(70);
    expect(result.graph.nodes).toHaveLength(164);
    expect(result.graph.edges).toHaveLength(317);
  });
