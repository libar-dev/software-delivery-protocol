...
describe("the self-hosting corpus", () => {
  ...
  it("holds the frozen corpus totals", () => {
    ...
    expect(result.counts).toEqual({ specs: 133, packs: 1, anchors: 124 });
    expect(expectedSpecs).toHaveLength(133);
    expect(expectedPackMembers).toHaveLength(133);
    expect(expectedAnchors).toHaveLength(124);
    expect(result.graph.nodes).toHaveLength(258);
    expect(result.graph.edges).toHaveLength(483);
  });
  ...
  it("holds the frozen stated-readiness distribution", () => {
    ...
    ).toEqual({ defined: 14, idea: 3, ready: 115, scoped: 1 });
  });
