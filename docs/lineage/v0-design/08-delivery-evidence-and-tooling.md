> **Lineage copy — restored for the executable-verification design review (`reviews/14`); never current truth.**
> This is the v0 concept document `08-delivery-evidence-and-tooling.md`, superseded 2026-06-06 by the reboot commit `ed5f6ad` and restored
> verbatim below from `ed5f6ad^`. It is design history kept as review evidence: on any disagreement with
> `specs/`, `docs/concept/`, and `src/`, those are authoritative and this document is the stale side, always.
> The drift rule applies to it, never from it.
