...
- **How thin an impact graph in the MVP.** The curated surface is MVP; whether a minimal import/symbol impact graph (for impact/blast-radius + curation-assist) ships in the first slice or lands as an Iterate-phase addition is a sequencing call. *(`06` §2.)*
