...
**Carrier addendum (post-MVP).** The CORE list above records what the MVP shipped — the TS DSL was then the only carrier. Specs and Packs now default to Markdown; the TS DSL survives as import source and a lawful per-ID option (the carrier ruling, MD-18, completed by the Pack syntax ruling, MD-25). Behavior and example Specs may instead use graph-aware Gherkin as a lawful per-ID option (the Gherkin carrier option, MD-27) with canonical suffix `.sdp.gherkin` (MD-28). New authoring reads the CORE list through this rule. Kind coverage beyond behavior/example, rich Gherkin content, and any default-carrier flip remain open under plan 29 brief A.

**ASPIRATIONAL:** runtime-observation overlay (the `observed` delivery fact; runtime observations, `Build`/`Deployment`/`Observation` nodes, `nfr-violated`); runtime-composition depth (Effect `R`, Awilix wiring, Fastify trees); harnesses + simulation; rich projections (LikeC4/OpenAPI/JSON-LD/SHACL); rich Spec Studio with scoped intent composition; AI slices + the **MCP surface** (designed-in, deferred build) + GraphRAG; architecture-enforcement checks; a fuller impact graph; incremental builds/caching; full CLI; `--lenient` ratchet; multi-tenant/multi-repo/polyglot.
...
3. **Gherkin authoring surface (cut from MVP; later re-admitted for two kinds).** *Rationale at cut:* a second parser + tag linter + round-trip; the TS DSL already expressed examples. A single canonical authoring surface per Spec ID — dual source is the biggest anti-pattern to avoid — remains the law. *Post-MVP:* the Gherkin carrier option (MD-27) admits graph-aware Gherkin as a lawful per-ID option for behavior and example Specs only; MD-28 settles the canonical suffix as `.sdp.gherkin`. Universality, rich content, and any default flip stay open under plan 29 brief A.
