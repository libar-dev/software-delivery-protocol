...
**Carrier addendum (post-MVP).** The CORE list above records what the MVP shipped — the TS DSL was then the only carrier. Specs and Packs now default to Markdown; the TS DSL survives as import source and a lawful per-ID option (the carrier ruling, MD-18, completed by the Pack syntax ruling, MD-25). New authoring reads the CORE list through this rule.
