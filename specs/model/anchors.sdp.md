...
- **structural anchor validity** — A `memberOf` source is an `impl:` or `api:` CodeNode and its target is a `component:` CodeNode; every structural target exists, every edge is unique, each source has at most one component, and structural self-reference is refused. Any failure excludes the whole code anchor rather than preserving a partial declaration. Multi-node `uses` cycles remain data and produce no finding.
