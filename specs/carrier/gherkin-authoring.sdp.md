...
## Behavior
- rule: One `.feature` file carries exactly one behavior Spec as its Feature and zero or more example Specs as ordinary Scenarios, with one canonical carrier surface per Spec ID.
...
- rule: Closed keyed description lines populate existing intent and verification fields, remaining prose belongs to narrative, unknown keys and heading-shaped lines are refused, and no Gherkin form is invented for open questions.
